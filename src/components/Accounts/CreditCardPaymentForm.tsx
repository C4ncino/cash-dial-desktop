...
import {
  createCreditCardPaymentRequests,
  getPaymentCoverage,
  type PaymentSourceRow,
  validateCreditCardPayment,
} from "@/lib/forms/creditCardPayment";
...
const CreditCardPaymentForm = ({
  ...
  const { amountCovered, remaining, isExactlyCovered } = getPaymentCoverage(rows, totalAmount);
  ...
  const onSubmit = async (e: React.SubmitEvent) => {
    ...
    const validation = validateCreditCardPayment(rows, totalAmount);
    if (!validation.valid) {
    ...
    const payments = createCreditCardPaymentRequests(rows, currency, (accountId) => {
      const sourceAccount = accountsStore.getState().getById(accountId);
      return sourceAccount ? currencyStore.getState().getById(sourceAccount.currencyId) : undefined;
    });
  ...
  return (
    <form
      className="space-y-4 border-t border-zinc-200 bg-zinc-100/60 p-4 backdrop-blur-md dark:border-zinc-800 dark:bg-zinc-900/60"
      onSubmit={onSubmit}
    >
      ...
      <div className="space-y-1 border-t border-zinc-200 pt-3 text-sm dark:border-zinc-800">
        ...
        <div className="flex justify-between">
          ...
          <span
            className={`font-medium ${isExactlyCovered ? "text-green-600 dark:text-green-400" : "text-zinc-700 dark:text-zinc-300"}`}
          >
        ...
        <div className="flex justify-between">
          ...
          <span
            className={`font-medium ${remaining === 0 ? "text-green-600 dark:text-green-400" : "text-yellow-600 dark:text-yellow-400"}`}
          >
